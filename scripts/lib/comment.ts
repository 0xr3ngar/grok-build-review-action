...
export const upsertStatusComment = async (
    ...
    if (commentId !== null) {
        const updated = await ghApi("PATCH", `/repos/${repo}/issues/comments/${commentId}`, {
            body,
        });
        if (updated.ok) return;
    }

    const created = await ghApi<IssueComment>(
        "POST",
    ...
    if (!created.ok) {
        console.log(
            `::warning::Could not post status comment (HTTP ${created.status}): ${created.error}`,
        );
        return;
    }
