...
const postIssuesReview = async (ctx: Ctx, summary: string, issues: Issue[]): Promise<string> => {
    ...
    let result = await ghApi<PostedReview>(
        "POST",
        `/repos/${ctx.repo}/pulls/${ctx.prNumber}/reviews`,
        {
    ...
    if (!result.ok && result.status === 422 && inline.length > 0) {
        console.log(
            `::warning::Inline comments rejected (422), posting body-only review: ${result.error}`,
        );
        const fallbackBody =
            `## Grok Build Review\n\n${summary}\n\n${countsLine(issues)}\n\n### Findings\n\n` +
            issues.map((issue) => renderIssue(issue, true)).join("\n\n---\n\n") +
            `\n\n${footer(ctx)}`;
        result = await ghApi<PostedReview>(
            "POST",
            `/repos/${ctx.repo}/pulls/${ctx.prNumber}/reviews`,
            {
                commit_id: ctx.headSha,
    ...
    console.log(
        `Posted review: ${inline.length} inline, ${promoted.length} promoted — ${reviewUrl}`,
    );
    return reviewUrl;
};
